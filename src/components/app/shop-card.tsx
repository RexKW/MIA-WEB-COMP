...
export function ShopCard({
  ...
  return (
    <Card
      className={cn(
        "relative overflow-hidden w-full max-w-md rounded-xl",
        variant === "flat" && "bg-transparent border-0 shadow-none",
        className
      )}
    ...
    >
      ...
      >
        ...
        <div className="absolute left-4 top-4">
          <Badge variant="default">{isOpen ? "Open" : "Closed"}</Badge>
        </div>

        {/* Favorite Star */}
        <button
          aria-label="add to favorites"
          className="absolute right-4 top-4 rounded-full bg-background/60 p-2 shadow-md"
        >
          <Star
            className={
              isFavorite
      ...
      <CardContent
        className={cn(
          "-mt-6 rounded-b-xl bg-primary px-6 py-8",
          compact ? "py-4" : "py-8"
        )}
      >
